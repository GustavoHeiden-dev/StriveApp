...
<%@ page
	import="Modelos.Usuario, java.util.List, Modelos.Treino, Modelos.Exercicio, java.util.stream.Collectors"%>
<%
...
boolean treinoFinalizado = session.getAttribute("treinoFinalizado") != null;
if (treinoFinalizado) {
	session.removeAttribute("treinoFinalizado");
}
%>
<!DOCTYPE html>
<html lang="pt-BR">
...
.modal-exercicio-list {
	list-style: none;
}

.modal-exercicio-list li {
	display: flex;
	align-items: center;
	padding: 1rem;
	border-radius: 8px;
...
.modal-exercicio-list li:hover {
	background-color: var(--bg-light);
}

.modal-exercicio-list li.selected {
	background-color: #eadaff;
	font-weight: 600;
}

.modal-exercicio-list input[type="checkbox"] {
	width: 18px;
	height: 18px;
	...
	<div class="dashboard-container">
		...
		<main class="main-content">
			<header class="main-header">
				<h1>Seus Treinos</h1>
				<p>Explore os treinos disponíveis ou crie uma nova rotina
					personalizada.</p>
			</header>
			<section id="formCard" class="card">
				...
				<form id="createTreinoForm" action="TreinoServlet" method="post">
					<input type="hidden" name="acao" value="criar"> <input
						type="hidden" name="idTreino" value="">
					<div class="form-group">
						<label for="nome">Nome do Treino</label> <input type="text"
							id="nome" name="nome" placeholder="Ex: Treino A" required>
					</div>
					<div class="form-group">
						...
						<button type="button" id="openModalBtn" class="btn btn-outline"
							style="margin-top: 1rem;">
							<i class="fas fa-plus"></i> Adicionar / Editar Exercícios
						</button>
					</div>
					<div id="hidden-inputs-container"></div>
					<button type="submit" class="btn" style="margin-top: 1.5rem;">Salvar
						Treino</button>
				</form>
			</section>
			<section class="card">
				...
				<div class="treinos-grid">
					...
					if (treinos != null && !treinos.isEmpty()) {
						...
						exerciciosIds = t.getExercicios().stream().map(e -> String.valueOf(e.getId()))
	...
	<div id="exercicioModal" class="modal-overlay">
		<div class="modal">
			...
			<div class="modal-body">
				<div class="form-group">
					<input type="text" id="searchExercicio"
				...
				<ul class="modal-exercicio-list">
					...
					%>
					<li data-id="<%=ex.getId()%>" data-name="<%=ex.getNome()%>"><input
						type="checkbox" id="ex-<%=ex.getId()%>"> <label
						for="ex-<%=ex.getId()%>" style="flex-grow: 1; cursor: pointer;">
							<%=ex.getNome()%> <small>(<%=ex.getGrupoMuscular()%>)
						</small>
					</label></li>
					<%
					}
