...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .sidebar { width: var(--sidebar-width); background-color: var(--bg-sidebar); height: 100vh; position: fixed; left: 0; top: 0; padding: 2rem 1rem; border-right: 1px solid #ddd; display: none; z-index: 1100; }
        ...
        .sidebar .nav-list a .icon { font-size: 1rem; width: 20px; text-align: center; }
        ...
        .card { background: var(--bg-card); border-radius: var(--border-radius); padding: 2rem; box-shadow: var(--shadow); margin-bottom: 2rem; }
        .card h2 { margin-top: 0; margin-bottom: 1.5rem; color: var(--text-dark); font-size: 1.5rem; border-bottom: 1px solid var(--border-color); padding-bottom: 1rem; }
        .btn { display: inline-block; background: var(--primary-color); color: #fff; padding: 12px 24px; border: none; border-radius: 8px; font-weight: 600; font-size: 1rem; text-decoration: none; text-align: center; transition: background-color 0.3s, transform 0.2s; cursor: pointer; }
        ...
        .form-group input, .form-group textarea, .form-group select { width: 100%; padding: 12px; border: 1px solid var(--border-color); border-radius: 8px; font-size: 1rem; font-family: 'Poppins', sans-serif; }
        .form-group input:focus, .form-group textarea:focus, .form-group select:focus { outline: none; border-color: var(--primary-color); box-shadow: 0 0 0 3px rgba(106, 13, 173, 0.15); }
        ...
        .bottom-nav { display: flex; justify-content: space-around; padding: 0.8rem 0; background-color: #f0f0f5; border-top: 1px solid #ddd; position: fixed; bottom: 0; left: 0; width: 100%; z-index: 1000; }
        ...
        @media (max-width: 768px) { .main-content { padding: 1.5rem; padding-bottom: 100px; } .main-header h1 { font-size: 2rem; } }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateSelectedDisplay() {
        ...
        if (selectedExercises.size === 0) {
            ...
            selectedExercises.forEach((name, id) => {
                ...
                removeIcon.className = 'fas fa-times-circle remove-exercise-btn';
    ...
    selectedContainer.addEventListener('click', (e) => {
        if (e.target.classList.contains('remove-exercise-btn')) {
            ...
            selectedExercises.delete(idToRemove);
            updateSelectedDisplay();
        }
    });
});
