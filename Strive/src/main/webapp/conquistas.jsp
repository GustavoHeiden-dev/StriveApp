...
<%
    ...
    conquistas.add(new Conquista("Assíduo", "Treine todos os dias por 7 dias seguidos.", "fas fa-calendar-day", 7, treinosConcluidos));
    conquistas.add(new Conquista("Sem Descanso", "Treine todos os dias por 30 dias seguidos.", "fas fa-calendar-week", 30, treinosConcluidos));
    ...
    for(Conquista c : conquistas){
        if(c.concluido) conquistasConcluidas.add(c);
        else conquistasAtivas.add(c);
    }
%>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .sidebar { width: var(--sidebar-width); background-color: var(--bg-sidebar); height: 100vh; position: fixed; left: 0; top:0; padding:2rem 1rem; border-right:1px solid #ddd; display:none; z-index:1001; }
        ...
        .main-content { flex:1; padding:2rem; padding-bottom:100px; margin-left:0; }
        .main-header { margin-bottom:2rem; }
        .main-header h1 { font-size:2.5rem; margin-bottom:0.5rem; }
        .main-header p { color: var(--text-muted); margin-bottom:1rem; }
        .conquistas-grid { display:grid; grid-template-columns: repeat(auto-fit,minmax(220px,1fr)); gap:1rem; }
        .conquista-card { background:var(--bg-card); border-radius:var(--border-radius); padding:1rem; display:flex; flex-direction:column; align-items:center; gap:0.5rem; position:relative; box-shadow: var(--shadow); }
        .conquista-icon { font-size:2rem; color:var(--primary-color); width:60px; height:60px; border-radius:50%; display:flex; align-items:center; justify-content:center; background:#f1ebff; }
        .conquista-info h3 { font-size:1rem; text-align:center; margin:0; }
        .conquista-info p { font-size:0.8rem; text-align:center; color: var(--text-muted); margin:0.25rem 0; }
        .progress-bar-container { width:80%; height:6px; background:#eee; border-radius:10px; overflow:hidden; margin-top:0.25rem; }
        .progress-bar { height:100%; width:0%; background:var(--primary-color); transition:width 0.5s; border-radius:10px; }
        .progress-bar.concluido { background: var(--gold-color); }
        ...
        .modal-content { background:#fff; padding:2rem; border-radius:12px; max-width:500px; width:90%; max-height:80%; overflow-y:auto; position:relative; }
        .modal h2 { margin-bottom:1rem; }
        .close-modal { position:absolute; top:10px; right:10px; cursor:pointer; font-size:1.2rem; }
        .parabens-toast { position:fixed; top:20px; left:50%; transform:translateX(-50%); background:var(--gold-color); color:#333; padding:1rem 2rem; border-radius:12px; display:none; font-weight:600; box-shadow: var(--shadow); z-index:10000; }
        .bottom-nav { display:flex; justify-content:space-around; padding:0.8rem 0; background:var(--bg-sidebar); border-top:1px solid #ddd; position:fixed; bottom:0; left:0; width:100%; z-index:100; }
        ...
        @media(min-width:992px){ .sidebar{display:block;} .main-content{margin-left:var(--sidebar-width);} .bottom-nav{display:none;} }
        @media(max-width:768px){ .main-header h1{font-size:2rem;} }
    </style>
</head>
<body>
    <div class="dashboard-container">
        <aside class="sidebar">
            ...
            <ul class="nav-list">
                ...
                <li><a href="ConquistasServlet" class="active"><i class="fas fa-trophy icon"></i> Conquistas</a></li>
        ...
        <main class="main-content">
            ...
            <div class="conquistas-grid">
                <% for(Conquista c : conquistasAtivas) { %>
                    <div class="conquista-card">
                        <div class="conquista-icon"><i class="<%=c.icone%>"></i></div>
            ...
            <button id="verConquistas" style="margin-top:2rem;padding:0.7rem 1.2rem;border:none;background:var(--primary-color);color:#fff;border-radius:8px;cursor:pointer;">Ver conquistas concluídas</button>
        </main>
        ...
        <nav class="bottom-nav">
            ...
            <a href="ConquistasServlet" class="active"><i class="fas fa-trophy icon"></i> Conquistas</a>
        ...
        <div class="modal-content">
            ...
            <ul>
                <% for(Conquista c : conquistasConcluidas) { %>
                    <li><i class="<%=c.icone%>" style="color:var(--gold-color);margin-right:0.5rem;"></i><%=c.nome%> - <%=c.descricao%></li>
                <% } %>
            </ul>
        </div>
    </div>

    <div class="parabens-toast" id="parabensToast">Parabéns! Conquista concluída!</div>
