<%@ page contentType="text/html;charset=UTF-8" import="Modelos.Usuario, java.util.List, Modelos.Exercicio, Modelos.Serie"%>
<%
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .main-content { flex: 1; padding: 1rem; padding-bottom: 100px; }
        .main-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem; flex-wrap: wrap; gap: 1rem; }
        .main-header h1 { font-size: 2rem; font-weight: 700; color: var(--dark-color); }
        .progress-counter { background-color: var(--primary-color); color: var(--white-color); padding: 0.5rem 1rem; border-radius: 20px; font-weight: 600; }
        .exercicio-card { background: var(--white-color); border-radius: var(--border-radius); padding: 1.5rem; box-shadow: var(--shadow); margin-bottom: 1.5rem; border-left: 5px solid var(--primary-color); transition: all 0.4s ease; }
        .exercicio-card.concluido { border-left-color: var(--success-color); background-color: var(--success-light-color); }
        .exercicio-card.concluido .exercicio-header h3 { color: var(--success-color); text-decoration: line-through; }
        .exercicio-card.concluido .grupo-muscular { background-color: var(--success-color); }
        ...
        .grupo-muscular { background: var(--secondary-color); color: white; padding: 4px 10px; border-radius: 20px; font-size: 0.8rem; font-weight: 600; white-space: nowrap; }
        .exercicio-details p { margin: 0.5rem 0; color: #555; }
        .btn { display: inline-block; background: var(--primary-color); color: var(--white-color); padding: 10px 20px; border: none; border-radius: 8px; font-weight: 600; text-decoration: none; transition: all 0.3s; cursor: pointer; }
        .btn:hover { background-color: var(--secondary-color); transform: translateY(-2px); }
        .btn-success { background-color: var(--success-color); }
        .finalizar-treino-container { text-align: center; margin-top: 2rem; padding: 2rem; background: var(--white-color); border-radius: var(--border-radius); box-shadow: var(--shadow); }
        .btn-finalizar { font-size: 1.2rem !important; padding: 15px 30px !important; width: 100%; max-width: 400px; }
        .btn-finalizar.disabled { background-color: var(--disabled-color); cursor: not-allowed; color: #888; }
        .btn-finalizar.disabled:hover { transform: none; }
        ...
        .series-section { margin-top: 1.5rem; padding-top: 1.5rem; border-top: 1px solid #eee; }
        .series-list { list-style: none; margin-bottom: 1.5rem; }
        .serie-item { display: flex; justify-content: space-between; background-color: var(--light-color); padding: 0.75rem 1rem; border-radius: 6px; margin-bottom: 0.5rem; font-weight: 500; }
        .serie-form .form-row { display: grid; grid-template-columns: 1fr 1fr auto auto; gap: 1rem; align-items: flex-end; }
        ...
        <main class="main-content">
            <div class="main-header">
                ...
                <div class="progress-counter" id="progressCounter">
                    <i class="fas fa-dumbbell"></i> <span id="completedCount">0</span>/<%=totalExercicios%> Concluídos
                </div>
            </div>
            ...
            <% if (exercicios != null && !exercicios.isEmpty()) { %>
                <% for (Exercicio ex : exercicios) { %>
                    <div class="exercicio-card" id="card-<%= ex.getId() %>">
                        ...
                        <div class="exercicio-details">
                            <p><strong>Meta Sugerida:</strong> <%= ex.getRepeticoes() %></p>
                        </div>
                        ...
                        <div class="series-section">
                            ...
                            <ul class="series-list" id="series-list-<%= ex.getId() %>">
                                <% if (ex.getSeries() != null && !ex.getSeries().isEmpty()) {
                                    for (Serie s : ex.getSeries()) { %>
                                        <li class="serie-item">
                                            <span><%= s.getRepeticoes() %> repetições</span>
                                            <span><%= s.getPeso() %> kg</span>
                                        </li>
                                    <% }
                                } else { %>
                                    <li class="no-series" style="text-align: center; color: var(--text-muted);">Nenhuma série registrada.</li>
                                <% } %>
                            </ul>
                            ...
                            <form class="serie-form" data-exercicio-id="<%= ex.getId() %>">
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="form-group">
                                        <label>Peso (kg)</label>
                                        <input type="number" step="0.5" name="peso" placeholder="50.5" required>
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        <button type="button" class="btn btn-concluir-exercicio" data-exercicio-id="<%= ex.getId() %>">
                ...
                <div class="finalizar-treino-container">
                    ...
                    <form action="FinalizarTreinoServlet" method="post" id="finalizarTreinoForm">
                        <input type="hidden" name="idSessao" value="<%= idSessao %>">
                        <input type="hidden" name="idTreino" value="<%= idTreino %>">
                        <button type="submit" id="finalizarBtn" class="btn btn-success btn-finalizar disabled">
